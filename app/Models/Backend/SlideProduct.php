...
class SlideProduct extends Model
{
    ...
    public function getAboutAttribute($about)
    {
        ...
        $option = ($this->option);
        if(isset($option->about->$l))
        {
            return $option->about->$l != ''?$option->about->$l:$about;
        }
    ...
    static function getPaginateSearch($request)
    {
        ...
        $q['about'] = $request->about;
        $q['description'] = $request->description;
        $q['status'] = $request->status;
        $_result = SlideProduct::orderBy('id','DESC');
        foreach ($q as $k=>$v)
        {
    ...
    static function saveForm($request)
    {
        $id = $request->id;
        if ($id != 0){
            $model = SlideProduct::find($id);
        }else{
            $model = new SlideProduct();
        }
