@php
    ...
    $page_type = isset($result->page_type)?$result->page_type:0;

@endphp

@extends('layouts.admin-form')

@section('title',_t('Page Form'))

@section('content')
    @component('component.form',['option' => ['url' => '/backend/page-save','id'=>'fileupload'],'title' => _t('Page Form')])
        {!! Form::hidden('id',$id) !!}
        ...
        <div class="form-body">
            <div class="row">
                @component('component.input', ['label' => _t('Page Type'),'sms' => ''])
                    {!! Form::select('page_type', page_type(), $page_type,['class'=>"form-control"]) !!}
                @endcomponent
            </div>
