        ...
        <div class="col-md-12">
            ...
            <div class="portlet">
                <div class="portlet-title">
                    <div class="caption">
                        <i class="fa fa-bell-o"></i>{{ _t('Page') }} </div>
                    <div class="tools">
                    ...
                    <div class="table-scrollable table-responsive">
                        <table class="table table-striped table-bordered table-advance table-hover">
                            ...
                            <tr>
                                ...
                                <th> {!! get_title_search_page_type('page_type',$page_type) !!} </th>
                                <th style="width: 180px;"> {!! get_title_search_status($status) !!}  </th>
                                <th></th>
                            </tr>
                            <tr>
                                <th style="width: 98px;">  <a href="{{ url('/backend/page-form') }}" class="btn btn-icon-only green">
                            ...
                            @foreach ($result as $row)
                                <tr>
                                    <td>
                                        {!! Form::open(['url' => "/backend/page-form", 'method' => 'put']) !!}
                                        {!! Form::hidden('id',$row->id) !!}
                                        {!! Form::hidden('_page',$page) !!}

                                        <button type="submit" class="btn btn-icon-only purple">
                                            <i class="fa fa-edit"></i></button>
                                        <a data-href="{{ url('/backend/page-delete') }}" data-id="{{  $row->id }}" href="javascript:" class="btn btn-icon-only red vm-del">
                                    ...
                                    <td class="hidden-xs">
                                        {!! Form::select('page_type',page_type(),$row->page_type,['class'=>"form-control status-ch",'data-id'=> $row->id,
                                        'data-url'=> url("/backend/page-update-page_type")]) !!}
                                    </td>
                                    <td class="hidden-xs">
                                        {!! Form::select('status',status(),$row->status,['class'=>"form-control status-ch",'data-id'=> $row->id,
                                        'data-url'=> url("/backend/page-update-status")]) !!}
                                    </td>
