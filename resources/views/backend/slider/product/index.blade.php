...
@section('title',_t('Product Slide Page'))
        ...
        <div class="col-md-12">
            <div class="portlet">
                <div class="portlet-title">
                    <div class="caption">
                        <i class="fa fa-bell-o"></i>{{ _t('Product Slide List') }} </div>
                    <div class="tools">
                    ...
                    <div class="table-scrollable table-responsive">
                        <table class="table table-striped table-bordered table-advance table-hover">
                            ...
                            <tr>
                                ...
                                <th> {!! get_title_search('about',$about) !!} </th>
                                <th> {!! get_title_search('description',$description) !!}  </th>
                                <th style="width: 180px;"> {!! get_title_search_status($status) !!}  </th>
                                <th></th>
                            </tr>
                            <tr>
                                <th style="width: 98px;">  <a href="{{ url('/backend/product-slider-form') }}" class="btn btn-icon-only green">
                            ...
                            @foreach ($result as $row)
                                <tr>
                                    <td>
                                        {!! Form::open(['url' => "/backend/product-slider-form", 'method' => 'put']) !!}
                                        {!! Form::hidden('id',$row->id) !!}
                                        {!! Form::hidden('_page',$page) !!}
                                        <button type="submit" class="btn btn-icon-only purple">
                                            <i class="fa fa-edit"></i></button>
                                        <a data-href="{{ url('/backend/product-slider-delete') }}" data-id="{{  $row->id }}" href="javascript:" class="btn btn-icon-only red vm-del">
                                    ...
                                    <td class="hidden-xs"> {!! highlight($row->about,$about) !!} </td>
                                    ...
                                    <td class="hidden-xs">
                                        ...
                                        'data-url'=> url("/backend/product-slider-update-status")]) !!}
                                    </td>
