        ...
        <div class="main container">
            <div class="breadcrumbs">
                <ul>
                    ...
                    <li><span>/</span>                            <a href="{{url('/my-account')}}">{{_t('Account')}}</a>
                    </li>
                    ...
                    <div class="col-main">
                        <div class="my-account">
                            <div class="page-title">
                                <h2>{{_t('Order History')}}</h2>
                            </div>
                            <div class="table-responsive">
                                <table class="table table-hover">
                                    <thead>
                                    <tr>
                                        <td class="text-right">{{_t('Order ID')}}</td>
                                        <td class="text-left">{{_t('Customer')}}</td>
                                        <td class="text-right">{{_t('Qty')}}</td>
                                        <td class="text-left">{{_t('Status')}}</td>
                                        <td class="text-right">{{_t('Total')}}</td>
                                        <td class="text-left">{{_t('Date Added')}}</td>
                                        <td></td>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    @foreach($OrderInfo as $info)
                                        <tr>
                                            <td class="text-right">#{{$info->id}}</td>
                                            <td class="text-left">{{$customer_info->name}}</td>
                                            <td class="text-right">{{$info->total_qty}}</td>
                                            <td class="text-left">@if(($info->paid) != 1 && ($info->cancel) != 1 ) <b style="color: red;">{{_t('Pending')}}</b> @endif @if(($info->paid) != 0) <b style="color: green;">{{_t('Paid')}}</b> @endif @if(($info->cancel) != 0 ) <b style="color: #ff7813;">{{_t('Cancel')}}</b> @endif</td>
                                            <td class="text-right">$ {{$info->total_price}}</td>
                                            <td class="text-left">{{$info->delivery_date}}</td>
                                            <td class="text-right">
                                                <a href="{{url('/order/history-info').'/'.$info->id}}" data-toggle="tooltip" title="" class="btn btn-info" data-original-title="View"><i class="fa fa-eye"></i></a></td>
                                        </tr>
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-6 text-right">  {{ $OrderInfo->links() }}</div>
                            </div>
                            <div class="buttons clearfix">
                                <div class="pull-right">
                                    <!-- <a href="" class="btn btn-primary"></a> -->
                                    <a href="{{url('/my-account')}}" class="button continue">{{_t('Continue')}}</a>
                                </div>
                            </div>
                        </div></div>
                </div>
