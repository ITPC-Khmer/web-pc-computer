        ...
        <div class="main container">
            <div class="breadcrumbs">
                <ul>
                    ...
                    <li><span>/</span>                            <a href="{{url('/my-account').'/'.Auth::id()}}">{{_t('Account')}}</a>
                    </li>
                    <li><span>/</span>                            <strong>{{_t('Order History')}}</strong>            </li>
                    ...
                    <div class="col-main">
                        <div class="my-account">
                            ...
                            <table class="table table-hover">
                                <thead>
                                <tr>
                                    <td class="text-left" colspan="2">{{_t('Order Details')}}</td>
                                </tr>
                                </thead>
                                <tbody>
                                <tr>
                                    <td class="text-left" style="width: 50%;">
                                        <b>{{_t('Order ID')}}:</b> <span style="background-color: yellow"> #{{$order_history_detail->id}}</span><br>
                                        <b>{{_t('Date Added')}}:</b> {{$order_history_detail->delivery_date}} <br>
                                        <b>{{_t('Time Added')}}:</b> {{$order_history_detail->delivery_time}}
                                    </td>
                                </tr>
                                </tbody>
                            </table>
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover">
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-left">{{_t('Model Code')}}</td>
                                        <td class="text-left">{{_t('Image')}}</td>
                                        <td class="text-right">{{_t('Quantity')}}</td>
                                        <td class="text-right">{{_t('Price')}}</td>
                                        <td class="text-right">{{_t('Total')}}</td>
                                        <td style="width: 20px;"></td>
                                    </tr>
                                    </thead>
                                    <tbody>
                                        @foreach(json_decode($order_history_detail->order_detail) as $item)
                                            <tr>
                                                <td class="text-left">{{$item->name}}</td>
                                                <td class="text-left">{{$item->options->code}}</td>
                                                <td class="text-left"><img src="{{$item->options->image}}" style="width: 50px; height: 50px;"></td>
                                                <td class="text-right">{{$item->qty}}</td>
                                                <td class="text-right">${{$item->price}}</td>
                                                <td class="text-right">${{$item->price*$item->qty}}</td>
                                                <td class="text-right" style="white-space: nowrap;">
                                                @if($order_history_detail->cancel-0!=0)
                                                        <a href="#" data-id="{{$item->id}}" data-code="{{$item->options->code}}" data-image="{{$item->options->image}}" data-price="{{$item->price}}"  data-name="{{$item->name}}"
                                                           data-toggle="tooltip" title="" class="desktop-addCart btn btn-primary" data-original-title="Reorder"><i class="fa fa-shopping-cart"></i></a>
                                    ...
                                    <tr>
                                        <td colspan="3"></td>
                                        <td class="text-right"><b>{{_t('Sub Total')}}</b></td>
                                        <td class="text-right">${{$order_history_detail->sub_total}}</td>
                                        <td></td>
                                    </tr>
                                    <tr>
                                        <td colspan="3"></td>
                                        <td class="text-right"><b>{{_t('TAX(0%)')}}</b></td>
                                        <td class="text-right">${{$order_history_detail->tax}}</td>
                                        <td></td>
                                    </tr>
                                    <tr>
                                        <td colspan="3"></td>
                                        <td class="text-right"><b>{{_t('Grand Total')}}</b></td>
                                        <td class="text-right">${{$order_history_detail->total_price}}</td>
                                        <td></td>
                                    </tr>
                                    </tfoot>
                                </table>
                            ...
                            <div class="buttons clearfix">
                                <div class="pull-right">
                                    ...
                                    <a href="{{url('/order/history').'/'.Auth::id()}}" class="button continue">{{_t('Continue')}}</a>
                                </div>
                            </div>
                        </div>
                    </div>
