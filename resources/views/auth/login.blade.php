            ...
            <div class="account-login">
                <div class="row">
                    <div class="breadcrumbs">
                        <ul>
                            <li>
                                <a href="{{url('/')}}">{{_t('Hom')}}e</a>
                            </li>
                            <li><span>/</span>
                                <a href="{{url('/account')}}">{{_t('Account')}}</a>
                            </li>
                            <li><span>/</span>
                                <strong>{{_t('Login')}}</strong>
                            </li>
                        </ul>
                    </div>
                    <div id="content" class="col-sm-12">
                        <div class="page-title">
                            <h2>{{_t('Account Login')}}</h2>
                        </div>
                        <div class="col2-set">
                            <div class="col-1 new-users">
                                <div class="content">
                                    <h2>{{_t('New Customer')}}</h2>
                                    <p><strong>{{_t('Register Account')}}</strong></p>
                                    <p>{{_t('By creating an account you will be able to shop faster, be up to date on an orders status, and keep track of the orders you have previously made')}}.</p>

                                    <a href="{{url('/register')}}" class="button create-account" type="button"><span>{{_t('Continue')}}</span></a>
                                </div>
                            </div>
                            <div class="col-2 registered-users">
                                <div class="content">
                                    <h2>{{_t('Returning Customer')}}</h2>
                                    <p><strong>{{_t('I am a returning customer')}}</strong></p>


                                    <form action="{{ route('login') }}" method="post" enctype="multipart/form-data">
                                        {{ csrf_field() }}
                                        <div class="form-group{{ $errors->has('email') ? ' has-error' : '' }}">
                                            <label class="control-label" for="input-email">{{_t('Email Address')}}</label>
                                        ...
                                        <div class="form-group{{ $errors->has('password') ? ' has-error' : '' }}">
                                            <label class="control-label" for="input-password">{{_t('Password')}}</label>
                                        ...
                                        <label class="rememberme mt-checkbox mt-checkbox-outline">
                                            <input type="checkbox"
                                                   name="remember" {{ old('remember') ? 'checked' : '' }}> {{_t('Remember Me')}}
                                            <span></span>
                                        </label>&nbsp;||&nbsp;
                                        <a href="{{ route('password.request') }}">{{_t('Forgot Password')}}</a>
